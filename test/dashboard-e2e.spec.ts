...
test('display day orders amount metric', async ({ page }) => {
  ...
  await expect(page.getByText('-5% over yesterday')).toBeVisible()
})
test('display month orders amount metric', async ({ page }) => {
  await page.goto('/', { waitUntil: 'networkidle' })
  await expect(page.getByText('400', { exact: true })).toBeVisible()
  await expect(page.getByText('25% over last month')).toBeVisible()
})
test('display month revenue metric', async ({ page }) => {
  await page.goto('/', { waitUntil: 'networkidle' })
  await expect(page.getByText('€2,509.33')).toBeVisible()
  await expect(
    page.getByText('5% over last month', { exact: true }),
  ).toBeVisible()
})
test('display month canceled orders metric', async ({ page }) => {
  ...
  await expect(
    page.getByText('-3% over last month', { exact: true }),
  ).toBeVisible()
})
