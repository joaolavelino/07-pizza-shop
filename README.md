...
## Use Case! Automatically redirect to the sign-in screen if not authorized

If the user is not authorized, it's sensible to automatically redirect him to the sign-in page.

To do so, we use the Axios Interceptors. Instad of intercepting the requeste as before, we intercept the response and define the outcomes based on the response, wheather it's successful or error.

This example is from the Pizza Shop Project
to do so we use a `useEffect` on the App Layout (it envolves all the pages that make the requests we're going to intercept).
...
To make it cleaner and easier to test, read and maintain, we keep the login on a dedicated file:

`/api/_authInterceptor.ts`
...
And only call the function on the `useEffect`
